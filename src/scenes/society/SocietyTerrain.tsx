...
/**
 * The Society terrain — community plateau.
 * Vertex-colored with distinct zones for paths, plaza, and garden areas.
 */
export function SocietyTerrain() {
  const { geometry, material } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let h = 0
      if (d > 28) h = -Math.min(8, (d - 28) * 0.5)
      h += fbm(x * 0.05, z * 0.05, 3) * 0.6
      h += Math.sin(x * 0.15) * Math.cos(z * 0.12) * 0.15
      if (d > 24) h += Math.sin(x * 0.22) * Math.cos(z * 0.18) * (d - 24) * 0.04
      pos.setY(i, h)

      const isPath = Math.abs(x) < 2.2 && z > 14 && z < 30
      const isPlaza = d < 10
