...
/**
 * The Gate terrain — elevated final plateau.
 * Vertex-colored with path, plaza, and cliff zones. Rises toward the Gate.
 */
export function GateTerrain() {
  const { geometry, material } = useMemo(() => {
    const geo = new THREE.PlaneGeometry(110, 110, 100, 100)
    ...
    for (let i = 0; i < count; i++) {
      ...
      let h = 0
      h += Math.max(0, (-z - 5) * 0.12)
      if (d > 28) h -= Math.min(6, (d - 28) * 0.45)
      h += fbm(x * 0.05, z * 0.05, 3) * 0.5
      h += Math.sin(x * 0.18) * Math.cos(z * 0.14) * 0.12
      if (d > 22) h += Math.sin(x * 0.25) * Math.cos(z * 0.2) * (d - 22) * 0.03
      pos.setY(i, h)

      const isPath = Math.abs(x) < 2.2 && z > -4 && z < 24
      const isPlaza = Math.hypot(x, z + 2) < 10
      const isEdge = d > 24 && d < 32
      const isCliff = d > 30
